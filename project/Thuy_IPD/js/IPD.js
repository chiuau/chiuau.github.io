...
document.addEventListener('DOMContentLoaded', function() {
   availableALG = [new TFT(), new Pavlov(), new GTFT(), new FBF(), new AllC(), new AllD(),  new Random(), new TFTT(),new RTFT(), new Grim(), new ZDExtortion2(), new ZDGTFT2()];
}, false);
...
function game(x,y,deviated_iter) {
	...
	var last1 = 2; var last2 = 2;
	for (var i=0;i<deviated_iter;i++) {
		 choice1 = listALG[x].receive(last1,last2);
		 choice2 = listALG[y].receive(last2,last1);
		 ...
		 x2 += choice2.toString();
		 if (i-delay >= 0) {	
		 	last1 = parseInt(x1[i-delay]);
		 	last2 = parseInt(x2[i-delay]);
		 }
	}
